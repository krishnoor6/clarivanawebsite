...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById("biotech-button").addEventListener("click", async () => {
  const text = document.getElementById("extracted-text").value;
  if (!text.trim()) return;
  const aiResult = document.getElementById("ai-result");
  const aiOutput = document.getElementById("ai-output");
  aiResult.style.display = "block";
  aiOutput.innerHTML = `<div style="text-align:center;padding:20px;opacity:0.5;font-size:12px;letter-spacing:2px">RUNNING BIOCHEMICAL ANALYSIS...</div>`;
  const res = await fetch("./ingredients.json");
  const json = await res.json();
  const analysis = await window.ClarivanaAnalyze(text, json.harmfulIngredients);
