...
async function extractTextFromImage(canvasEl) {
  ...
  Tesseract.recognize(canvasEl, 'eng', { logger: m => console.log(m) })
    .then(({ data }) => {
      ...
      if (aiBtn) aiBtn.style.display = 'inline-block';
      const biotechBtn = document.getElementById('biotech-button');
      if (biotechBtn) biotechBtn.style.display = 'inline-block';
...
function resetScanSession() {
  ...
  setTimeout(() => {
    ...
    ['scan-another','ai-button','biotech-button'].forEach(id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
    document.getElementById('edit-button').style.display = 'inline';
    document.getElementById('save-button').style.display = 'none';
    startCamera();
